...
async function startScan() {
    ...
    const community = document.getElementById('g-community').value.trim() || 'public';
    ...
    try {
        ...
        evtSource.onmessage = (e) => {
            ...
            if (item.type === 'result') {
                if (item.ip) {
                    const snmpBadge = item.snmp === 'NO SNMP'
                        ? `<span class="badge badge-danger">${item.snmp}</span>`
                        : `<span class="badge badge-info">${item.snmp}</span>`;
